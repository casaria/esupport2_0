...
startTable("$lang_ticket $lang_priorities", "center", "100%", 1);
	...
	echo '</tr></td>
		 <tr><td class=back2>
		'.$lang_addpriority.': 
		<inout type=text class=cas-tb1 text-2  name=priority></input><br>
		'.$lang_responsetime.': <inout type=text class=cas-tb1 text-2  name=response size=2></input> '.$lang_hours.'<br>
...
function listtPriorities()
{
	...
	if($num_rows != 0){
		...
		while($row = $db->fetch_array($result)){
			...
			echo "<inout type=text class=cas-tb1 text-2  name=priority$i value=\"".$row['priority']."\">";
			echo "&nbsp;&nbsp; $lang_responsetime: <inout type=text class=cas-tb1 text-2  size=2 value='".toHours($row['response_time'])."' name=response".$i."> $lang_hours ";
