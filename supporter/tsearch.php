...
if(isset($search) || isset($s)) {
    ...
    if ($stmt == '') {
        ...
        if ($s == 'tim' || $s == 'mit') {
            $query = preg_replace("/select \* from $mysql_tickets_table where \(/i", "select t.*, ($time - t.lastupdate)/p.response_time as a from $mysql_tickets_table t, 
              $mysql_tpriorities_table p where p.priority=t.priority and (", $query);
        } else {
    ...
    if ($sql == "select * from $mysql_tickets_table") {
        ...
        echo "<tr><td><td class=back>
							<inout type=text class=cas-tb1 text-2  size=52% name=csvlist value='$summary[tktlist]'>
		</td></td></tr>";
        endTable();
       // endTable();
    }
	...
	echo '				</select>
						...
						<td class=back>
							<inout type=text class=cas-tb1 text-2  name=user>
						</td>
						</tr>
						<TR>
						<TD class=back2 align=right>'.$lang_office.': </td>
						<td class=back>
							<inout type=text class=cas-tb1 text-2  name=office>
						</td>
						</tr>
						<TR>
						<TD class=back2 align=right>'.$lang_betweenCreateDates.': </td>
						<td class=back>
						<select name=smonth>';
						for($i=1; $i<13; $i++){
							echo "<option value=$i";
