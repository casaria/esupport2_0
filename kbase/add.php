...
if($add == $lang_addtokb)
{
	...
	startTable("$lang_addtokb", "center", 100, 2);
		echo '<tr><td class=back2 align=right width=20%>'.$lang_platform.':</td> <td class=back>';
		echo '<select  name=platform>';
			createPlatformMenu(0, $platform);
		echo '</select></td></TR><TR>';
		echo '<td align=right class=back2 width=20%>'.$lang_category.': </td> <td class=back>';
		echo '<select name=category>';
			createKCategoryMenu(0, $category);
		echo '</select></td></tr>';
		echo '<tr><td class=back2 align=right width=20%>
			'.$lang_question.': </td><td colspan=1 class=back><inout type=text class=cas-tb1  size=60 name=question value="'.$short.'"></input></td></tr>';
		...
		echo '<inout type=text class=cas-tb1  size=60 name=keywords></input></td></tr>';
		...
		if($enable_kattachments == 'On'){
			...
			echo '<tr><td class=back2 align=right width=20%>'.$lang_addattachment.':</td> <td colspan=3 class=back>';
